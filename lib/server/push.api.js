...
Push.Configure = function(options) {
    ...
    if (options.apn) {
        ...
        apnConnection.on('error',  Meteor.bindEnvironment(function (errCode, notification, recipient) {
            // Failure to send a push notification should not crash server, need a better fix later
            console.log('APN Connection Error');
        }));
